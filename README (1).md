...
**Success factors:**
- Explicit KPI/SQL logic defined in Week 1 — don't leave definitions vague
- Ask clarification questions rather than let the LLM hallucinate schema entities
- Deploy a mock container to Cloud Run by Week 3 — don't leave deployment to the last week

**Risks & mitigations:**
- *SQL hallucination* → strict schema validation + whitelist syntax checking
- *BigQuery cost spikes* → result caching + byte-scan limits on all queries
- *Integration bottlenecks* → standardized JSON schemas + mock endpoints between frontend/backend from day 1

---

## 8. Target delivery metrics

- **90%+ SQL execution accuracy** across 30+ golden benchmark questions
- **2+ automated retail KPIs** computed reliably
- **<3s query-to-chart latency** end to end

---

## 9. Setup checklist

- [x] Create shared GitHub repo, add this README
- [x] Set up GitHub Projects board with 4 columns (Backlog / In Progress / Review / Done)
- [x] Create GitHub Issues, labeled by stage
- [x] Add `.gitignore` and `requirements.txt`
- [ ] Assign owners to the Stage 0 issues (one person per category)
- [ ] Everyone completes their Stage 0 category, then close the stage
- [ ] Stage 1: write `schema.md` and `api.md` together, create `/src`, `/mock_data`, `/docs` folders
- [ ] First Excalidraw architecture sketch, link it in `docs/architecture.md`
