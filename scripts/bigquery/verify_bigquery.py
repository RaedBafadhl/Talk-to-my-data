...
try:
    ...
    print(
        "Check the DATASET name at the top of this script matches what Fares actually used."
    )
    exit(1)
...
for table_name, expected in EXPECTED_TABLES.items():
    ...
    if actual_rows == expected["expected_rows"]:
        ...
        print(
            f"[MISMATCH] Row count is {actual_rows}, expected {expected['expected_rows']}"
        )
        all_good = False
...
if all_good:
    ...
    print(
        "SOME CHECKS FAILED -- see [MISMATCH]/[MISSING]/[FAIL] above before marking 1.2 done."
    )
print("=" * 70)
